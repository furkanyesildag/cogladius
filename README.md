...
>
> **Status:** Stellar **testnet** · real testnet **USDC** via the SAC · contract live + verified · 11 `testutils` tests green · Next.js app builds clean. Nothing is mocked — every settlement is a real on-chain transaction.
...
- 🧑‍⚖️ **Real three-judge LLM panel** — Technical · Usability · Completeness, scored by real LLM calls (Anthropic / OpenAI). No mock/random scores.
...
| `post_task(poster, task_id, reward, deadline)` | `poster` | Pulls `reward` USDC into the contract via the SAC; status → Open |
| `activate(task_id)` | `admin` | Open → Active (first submission recorded) |
| `release_to_winner(task_id, winner, score, nonce, signature)` | ed25519 verdict | Verifies the verdict signature, pays the winner if `score ≥ 70`; → Completed |
| `refund(task_id)` | poster (pre-deadline) / open (post-deadline) | Returns the reward to the poster; → Refunded |
| `flag_disputed(task_id)` | `admin` | Completed → Disputed (state stub; full resolution deferred) |
| `get_task` / `get_config` | — | Views |

**Design notes:** config lives in **instance storage** (idiomatic Soroban); persistent task entries use a 30-day TTL; events use the `#[contractevent]` macro; `release_to_winner` is permissionless but gated by the signature + a `nonce` + the `Completed` status, which together prevent replay and double-settle.

## Verdict authority (on-chain ed25519)

The three-judge LLM panel runs off-chain. Its averaged verdict is signed by a verdict-authority ed25519 key whose **public key is stored in the contract** (`verdict_pubkey`). `release_to_winner` re-derives the canonical message and verifies the signature on-chain — funds move only on a genuine, unforgeable verdict.
...
- **Posting** — [`app/lib/sorobanEscrow.ts`](./app/lib/sorobanEscrow.ts) builds the `post_task` invocation, simulates + assembles it, signs through **Freighter** (SEP-43), and submits to Soroban RPC. One connection, one signature, reward locked.
...
| `VERDICT_AUTHORITY_SECRET` | Server — its raw ed25519 pubkey is baked into the contract |
| `SOROBAN_SUBMITTER_SECRET` | Server — funded account that pays fees / is the release source |
...
- `release_to_winner` is permissionless but requires a valid verdict-authority **ed25519 signature** + an unused `nonce`; the `Completed` status blocks double-settlement.
- Secrets (`VERDICT_AUTHORITY_SECRET`, `SOROBAN_SUBMITTER_SECRET`) live in gitignored `.env.local` — never committed.
