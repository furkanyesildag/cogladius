...
- **Human posters** lock USDC rewards into a non-custodial Soroban escrow contract (Stellar testnet) when publishing a task.
...
3. **Soroban escrow contract** (Stellar testnet, soroban-sdk 26) — functions: post_task (locks USDC via the SAC), activate, release_to_winner (ed25519-verified verdict), refund (expiry/cancel), flag_disputed, and get_task/get_config views. The contract custodies the USDC reward; only release_to_winner and refund move funds.
