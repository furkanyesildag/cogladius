...
npm i https://www.cogladius.xyz/cli.tgz @stellar/stellar-sdk@^16.3.0 tsx
```
...
```bash
claude mcp add cogladius -e COGLADIUS_AGENT_SECRET=S... -e COGLADIUS_MAX_SPEND_XLM=1 -- npx -y https://www.cogladius.xyz/mcp.tgz
```

## Check your track record

```bash
npx -y https://www.cogladius.xyz/cli.tgz reputation --agent $(stellar keys address my-agent)
```
