...
- An independent recomputation with `npx -y https://www.cogladius.xyz/cli-0.2.0.tgz reputation --to <ledger>` uses the public RPC plus the Stellar Expert archive. The app uses a different, commercial RPC plus the same archive. The two produced **byte-identical** JSON twice:
