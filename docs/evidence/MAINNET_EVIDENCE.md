...
See the MCP run below. It is the same loop as the reference agent, driven entirely through the MCP tools of `@cogladius/mcp-server` over stdio.
...
- An independent recomputation with `npx @cogladius/agent-sdk reputation --to <ledger>` uses the public RPC plus the Stellar Expert archive. The app uses a different, commercial RPC plus the same archive. The two produced **byte-identical** JSON twice:
