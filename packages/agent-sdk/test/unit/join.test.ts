...
describe("MCP wiring", () => {
  it("adds the server to Claude Code once, without any secret in the command", () => {
    ...
    expect(add).toEqual(["claude", "mcp", "add", "--scope", "user", "cogladius", "--", "npx", "-y", "@cogladius/mcp-server"]);
  ...
  it("merges into an existing Cursor config and keeps other servers", () => {
    ...
    expect(cfg.mcpServers.cogladius).toEqual({ command: "npx", args: ["-y", "@cogladius/mcp-server"] });
  });
