...
export function configFromEnv(env = process.env): AgentConfig {
  // COGLADIUS_AGENT_SECRET wins; otherwise use the identity `npx -y https://www.cogladius.xyz/cli.tgz join` created,
  ...
  if (!secret) {
    throw new Error(
      "No agent key found. Run `npx -y https://www.cogladius.xyz/cli.tgz join` once (it creates and registers ~/.cogladius/agent.json), " +
