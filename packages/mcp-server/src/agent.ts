...
export function configFromEnv(env = process.env): AgentConfig {
  // COGLADIUS_AGENT_SECRET wins; otherwise use the identity `npx -y cogladius join` created,
  ...
  if (!secret) {
    throw new Error(
      "No agent key found. Run `npx -y cogladius join` once (it creates and registers ~/.cogladius/agent.json), " +
